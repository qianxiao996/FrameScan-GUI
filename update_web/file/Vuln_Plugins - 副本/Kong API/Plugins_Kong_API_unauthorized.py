...
def vuln_info():
    info={
        'vuln_name': 'Kong Admin Rest API未授权访问漏洞',  #漏洞名称
        'vuln_referer':'https://aqzt.com/6233.html',  #漏洞来源
        'vuln_author':'qianxiao996',  #插件作者
        'cms_name':'Kong API',#cms_name需要和上级目录保持一致。扫描器自动添加会调用。GUI版本不会调用
        'vuln_description':'''Kong API 网关是目前最受欢迎的云原生 API 网关之一，它通过插件的形式提供负载均衡等多重功能。Kong 使用 Kong Admin Rest API 作为管理 Kong Proxy 能力的关键入口。在 2.0.3 版本及其之前，当企业遵循官方文档或默认配置使用 Docker 容器的方式搭建 Kong API 网关时，会在默认情况下将未经鉴权的Admin Rest API 对公网开放，导致攻击者可以通过未授权访问Admin Rest API，从而控制Kong API网关。本站安全专题提醒 Kong API 网关用户尽快采取安全措施阻止漏洞攻击。''',
        'vuln_identifier':'''未知''',
        ...
        'vuln_solution':'''CVE-2020-11710''',
        'FofaQuery_type':'http', #socket、http
        'FofaQuery_link':'/', #此处的路径会加在url拼接访问，进行FofaQuery的条件匹配 此处为all为全部页面都检测
        'FofaQuery_rule':'body="plugins"',#header="JSESSIONID" || body="Struts Problem Report" || body="There is no Action mapped for namespace" || body="No result defined for action and result input" || header="Servlet" || header="JBoss",port="60001"
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        # print(r.text)
        if r.status_code == 200 and b"plugins" in r.content and b"configuration" in r.content:
            result['Result']= True
